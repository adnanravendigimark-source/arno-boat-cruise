      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <div className="mt-12 grid gap-10 lg:grid-cols-2 lg:items-start">
          <div className="rounded-2xl border border-stone-200/80 bg-white p-7 shadow-sm">
            ...
            <ol className="mt-6 space-y-6 border-l-2 border-forest-500/30 pl-6">
              {s.timeline.map((row, i) => (
                <li key={row.time + i} className="relative">
                  <span className="absolute -left-[31px] top-1 h-3.5 w-3.5 rounded-full bg-forest-600 ring-4 ring-forest-100" />
                  <span className="text-xs font-bold uppercase tracking-wider text-forest-600">{row.time}</span>
          ...
          <div className="space-y-6">
            <div className="rounded-2xl border border-stone-200/80 bg-white p-7 shadow-sm">
              ...
              <ul className="mt-5 space-y-3">
                {s.learn.map((item, i) => (
                  <li key={i} className="flex items-start gap-3 rounded-xl bg-forest-50/70 border border-forest-100 p-3.5 text-sm text-stone-700">
          ...
          <div className="mt-12">
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-3">
              {s.extraItems.map((point, i) => (
                <div key={point.name + i} className="rounded-2xl border border-stone-200/80 bg-white p-5 shadow-sm transition hover:border-forest-400/40">
                  <p className="text-sm font-bold text-forest-600">{point.name}</p>
        ...
        <div className="mt-12 flex flex-col items-start gap-4 rounded-2xl bg-gradient-to-r from-stone-900 to-stone-800 p-8 text-white shadow-xl sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-xs text-sage-200 mt-0.5">Flexible departure times from Piazza Mentana, Lungarno Torrigiani &amp; Ponte Vecchio docks</p>
          </div>
          <a
            href={s.ctaHref}
            className="shrink-0 rounded-xl bg-gradient-to-r from-forest-600 via-sage-600 to-forest-800 px-7 py-3 text-sm font-bold text-white shadow-md transition-all hover:scale-[1.02]"
          >
            {s.ctaButtonText}
          </a>
        </div>
