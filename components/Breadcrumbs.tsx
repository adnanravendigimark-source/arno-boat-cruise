...
export default function Breadcrumbs({ items }: { items: BreadcrumbItem[] }) {
  ...
  return (
    <>
      ...
      />
      <nav aria-label="Breadcrumb" className="mx-auto max-w-6xl px-4 pt-4 sm:px-6 text-xs text-slate-500 font-medium">
            ...
            return (
              <li key={item.path + item.name} className="inline-flex items-center gap-1.5">
                ...
                {isLast ? (
                  ...
                  <Link href={item.path} className="hover:text-forest-600 transition-colors">
                    {item.name}
                  </Link>
                )}
              </li>
